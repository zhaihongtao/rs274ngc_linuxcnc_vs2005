...
#include "canon.h"		// _parameter_file_name
 ...
 const char * tool_file_name)   /* name of tool file */
{
  ...
  {
	  sprintf_s ( buffer, "%s\\%s", szfile, tool_file_name );
	  nRet = loadToolTable(buffer, _tools, 0, 0);
  }
...
int main_test (int argc, char ** argv)
{
  ...
  char default_name[] = RS274NGC_PARAMETER_FILE_NAME_DEFAULT;
  int print_stack;
  int go_flag;
  char *inifile = NULL;
  int log_level = -1;
