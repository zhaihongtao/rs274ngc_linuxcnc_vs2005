...
// error_text array, but stop at max_size if the text is longer.
 ...
 void ERS_FUNC (const char *fmt, ...);

 void _setError (const char *fmt, va_list & ap);
...
#define NAMEDPARAMS_MODULE "namedparams"
 ...
 void DBG(int level,const char *fmt, va_list & ap);  
 // print to if RS274NGC/LOG_LEVEL > 1:

 void MSG_DBG(const char *fmt,...); 
 void do_Log(unsigned int flags, const char *file, int line,
	 const char *fmt, va_list & ap);

	 void _logDebug ( unsigned int mask, int dlflags,int level, const char *fmt, va_list & ap);

	 //void _logDebug ( unsigned int mask, int dlflags,int level, const char *fmt, va_list & ap);
	 void logDebug (const char *fmt, ...);
	 void logRemap (const char *fmt, ...);
	 void logNP (const char *fmt, ...);
	 void logPy (const char *fmt, ...);
	 void logConfig (const char *fmt, ...);
	 void logOword (const char *fmt, ...);
