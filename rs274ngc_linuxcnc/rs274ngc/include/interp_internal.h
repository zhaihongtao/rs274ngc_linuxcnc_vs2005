...
#define ERS_TWO(fmt,fir,sec)                                      \
	do {                                                   \
	setError (fmt,fir,sec);                    \
	_setup.stack_index = 0;                            \
	strncpy_s(_setup.stack[_setup.stack_index], __FUNCTION__, STACK_ENTRY_LEN); \
	_setup.stack[_setup.stack_index][STACK_ENTRY_LEN-1] = 0; \
	_setup.stack_index++; \
...
#define ERS_THR(fmt,fir,sec,thr)                                      \
	do {                                                   \
	setError (fmt,fir,sec,thr);                    \
	_setup.stack_index = 0;                            \
	strncpy_s(_setup.stack[_setup.stack_index], __FUNCTION__, STACK_ENTRY_LEN); \
	_setup.stack[_setup.stack_index][STACK_ENTRY_LEN-1] = 0; \
	_setup.stack_index++; \
...
#define ERS_FOR(fmt,fir,sec,thr,fur)                                      \
	do {                                                   \
	setError (fmt,fir,sec,thr,fur);                    \
	_setup.stack_index = 0;                            \
	strncpy_s(_setup.stack[_setup.stack_index], __FUNCTION__, STACK_ENTRY_LEN); \
	_setup.stack[_setup.stack_index][STACK_ENTRY_LEN-1] = 0; \
	_setup.stack_index++; \
...
#define ERS_FIV(fmt,fir,sec,thr,fur,fiv)                                      \
	do {                                                   \
	setError (fmt,fir,sec,thr,fur,fiv);                    \
	_setup.stack_index = 0;                            \
	strncpy_s(_setup.stack[_setup.stack_index], __FUNCTION__, STACK_ENTRY_LEN); \
	_setup.stack[_setup.stack_index][STACK_ENTRY_LEN-1] = 0; \
	_setup.stack_index++; \
...
//void CHKS ( bool bad, const char *fmt, ... );
//#define CHKS(bad, fmt, ...)                                    \
//     do {                                                       \
//         if (bad) {                                             \
// 	    ERS(fmt, ...);                          \
...
#define CHKS_ONE(bad, fmt,err)                                    \
	do {                                                       \
	if (bad) {                                             \
	ERS_ONE(fmt,err);                          \
	}                                                      \
	} while(0)
#define CHKS_TWO(bad, fmt,fir,sec)                                    \
	do {                                                       \
	if (bad) {                                             \
	ERS_TWO(fmt,fir,sec);                          \
	}                                                      \
...
// oword warnings 
#define OERR_TWO(fmt,fir,sec)                                      \
    do {						    \
	if (FEATURE(OWORD_WARNONLY))			    \
	    fprintf(stderr,fmt,fir,sec);	    \
	else						    \
	    ERS_TWO(fmt,fir,sec);			    \
    } while(0)

#define OERR_THR(fmt,fir,sec,thr)                                      \
	do {						    \
	if (FEATURE(OWORD_WARNONLY))			    \
	fprintf(stderr,fmt, fir,sec,thr);	    \
	else						    \
	ERS_THR(fmt, fir,sec,thr);			    \
	} while(0)

#define OERR_FOR(fmt,fir,sec,thr,fur)                                      \
	do {						    \
	if (FEATURE(OWORD_WARNONLY))			    \
	fprintf(stderr,fmt, fir,sec,thr,fur);	    \
	else						    \
	ERS_FOR(fmt, fir,sec,thr,fur);			    \
	} while(0)
#define OERR_FIV(fmt,fir,sec,thr,fur,fiv)                                      \
	do {						    \
	if (FEATURE(OWORD_WARNONLY))			    \
	fprintf(stderr,fmt, fir,sec,thr,fur,fiv);	    \
	else						    \
	ERS_FIV(fmt, fir,sec,thr,fur,fiv);			    \
	} while(0)
...
#define CYCLE_MACRO(call) for (repeat = block->l_number; \
     ...
     { \
       ...
       if(radius_increment) { \
           double radius, theta; \
           CHKS_STR((bb == 0 && aa == 0), ("Incremental motion with polar coordinates is indeterminate when at the origin")); \
           theta = atan2(bb, aa); \
           radius = _hypot(bb, aa) + radius_increment; \
           aa = radius * cos(theta); \
           bb = radius * sin(theta); \
       } \
       if(theta_increment) { \
