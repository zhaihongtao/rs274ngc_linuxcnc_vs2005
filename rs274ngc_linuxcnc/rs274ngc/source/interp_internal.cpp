...
int Interp::close_and_downcase(char *line)       //!< string: one line of NC code
{
    ...
    CHKS_STR((comment), NCE_UNCLOSED_COMMENT_FOUND);
    line[n] = 0;
    return INTERP_OK;
}
                         ...
                         setup_pointer settings)        //!< pointer to machine settings      
{
  ...
  if(block->radius_flag || block->theta_flag) {
      ...
      CHKS_STR((!_readers[(int)'x'] || !_readers[(int)'y']), ("Cannot use polar coordinate on a machine lacking X or Y axes"));
      CHKS_STR(((settings->plane != CANON_PLANE_XY)), ("Cannot use polar coordinate except in G17 plane"));
      CHKS_STR(((block->x_flag)), ("Cannot specify both polar coordinate and X word"));
      CHKS_STR(((block->y_flag)), ("Cannot specify both polar coordinate and Y word"));
  }
  ...
  if (mode1 != -1) {
    if (mode1 == G_80) {
      CHKS_STR(((polar_flag || axis_flag) && (!mode_zero_covets_axes)),
          NCE_CANNOT_USE_AXIS_VALUES_WITH_G80);
      CHKS_STR((polar_flag && mode0 == G_92), ("Polar coordinates can only be used for motion"));
      CHKS_STR(((!axis_flag) && (mode0 == G_52 || mode0 == G_92)),
	   NCE_ALL_AXES_MISSING_WITH_G52_OR_G92);
    } else {
      CHKS_STR(mode_zero_covets_axes,
          NCE_CANNOT_USE_TWO_G_CODES_THAT_BOTH_USE_AXIS_VALUES);
      CHKS_STR(((!axis_flag && !polar_flag) && 
            mode1 != G_0 && mode1 != G_1 && 
    ...
    CHKS_STR(((settings->motion_mode == -1)
  ...
  CHKS_STR((polar_flag && block->motion_to_be == -1), ("Polar coordinates can only be used for motion"));
  return INTERP_OK;
}
