                           ...
                           double *right)       //!< pointer to the right operand   
{
  switch (operation) {
  case DIVIDED_BY:
    CHKS_STR((*right == 0.0), NCE_ATTEMPT_TO_DIVIDE_BY_ZERO);
    *left = (*left / *right);
    break;
  case MODULO:                 /* always calculates a positive answer */
    *left = fmod(*left, *right);
    ...
    break;
  case POWER:
    CHKS_STR(((*left < 0.0) && (floor(*right) != *right)),
        NCE_ATTEMPT_TO_RAISE_NEGATIVE_TO_NON_INTEGER_POWER);
    *left = pow(*left, *right);
    break;
  case TIMES:
    *left = (*left * *right);
    break;
  default:
    ERS_STR(NCE_BUG_UNKNOWN_OPERATION);
  }
  return INTERP_OK;
}
			  ...
			  setup_pointer settings) //!< pointer to machine settings
{
  ...
  if ((block->g_modes[GM_SPINDLE_MODE] != -1) && ONCE(STEP_SPINDLE_MODE)) {
	  ...
	  if (block->dollar_flag){
		  CHKS_STR((block->dollar_number < 0 || block->dollar_number >= settings->num_spindles),
				  (("Invalid spindle ($) number in Spindle Mode command")));
  ...
  if ((block->g_modes[GM_FEED_MODE] != -1) && ONCE(STEP_FEED_MODE)) {
	  ...
	  if (block->dollar_flag){
		  CHKS_STR((block->dollar_number < 0 || block->dollar_number >= settings->num_spindles),
				  (("Invalid spindle ($) number in Spindle Feed command")));
		  settings->active_spindle = (int)block->dollar_number;
	  }
                         ...
                         int operation) //!< integer code for the operation 
{
  ...
  case ABS:
    ...
    break;
  case ACOS:
    CHKS_STR(((*double_ptr < -1.0) || (*double_ptr > 1.0)),
  ...
  case FUP:
    *double_ptr = ceil(*double_ptr);
    break;
  case LN:
    CHKS_STR((*double_ptr <= 0.0), NCE_ZERO_OR_NEGATIVE_ARGUMENT_TO_LN);
  ...
  case SIN:
    *double_ptr = sin((*double_ptr * M_PIl) / 180.0);
    break;
  case SQRT:
    CHKS_STR((*double_ptr < 0.0), NCE_NEGATIVE_ARGUMENT_TO_SQRT);
