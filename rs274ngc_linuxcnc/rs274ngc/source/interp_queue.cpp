...
void enqueue_M_USER_COMMAND (int index, double p_number, double q_number) {
    ...
    q.data.mcmd.index    = index;
    q.data.mcmd.p_number = p_number;
    q.data.mcmd.q_number = q_number;
...
int Interp::move_endpoint_and_flush(setup_pointer settings, double x, double y) {
    ...
    for(unsigned int i = 0; i<qc().size(); i++) {
        ...
        case QARC_FEED:
            double r1, r2, l1, l2;
            r1 = _hypot(q.data.arc_feed.end1 - q.data.arc_feed.center1,
            ...
            r2 = _hypot(x - q.data.arc_feed.center1,
            ...
            if(fabs(r1-r2) > .01) 
                ERS_TWO(("BUG: cutter compensation has generated an invalid arc with mismatched radii r1 %f r2 %f\n"), r1, r2);
            if(l1 && endpoint_valid && fabs(l2) > fabs(l1) + (settings->length_units == CANON_UNITS_MM? .0254 : .001)) {
                ERS_STR(("Arc move in concave corner cannot be reached by the tool without gouging"));
            }
            q.data.arc_feed.end1 = x;
            q.data.arc_feed.end2 = y;
            break;
        case QSTRAIGHT_TRAVERSE:
            ...
            case CANON_PLANE_XZ:
                ...
                break;
            default:
                ERS_STR(("BUG: Unsupported plane in cutter compensation"));
            }
            ...
            if(endpoint_valid && dot<0) {
                ...
                ERS_STR(("Straight traverse in concave corner cannot be reached by the tool without gouging"));
            }
            switch(settings->plane) {
            case CANON_PLANE_XY:
                q.data.straight_traverse.x = x;
        ...
        case QSTRAIGHT_FEED: 
            ...
            case CANON_PLANE_XZ:
                ...
                break;
            default:
                ERS_ONE(("BUG: Unsupported plane [%d] in cutter compensation"),
			settings->plane);
            }
            ...
            if(endpoint_valid && dot<0) {
                ...
                // path has crossed because we backed up further
                // than the line is long.  this will gouge.
                ERS_STR(("Straight feed in concave corner cannot be reached by the tool without gouging"));
            }
            switch(settings->plane) {
            case CANON_PLANE_XY:
                q.data.straight_feed.x = x;
