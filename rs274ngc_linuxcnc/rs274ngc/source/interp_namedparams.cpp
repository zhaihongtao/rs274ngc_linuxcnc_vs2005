				 ...
				 bool check_exists)    //!< test for existence, not value
{
	...
	*double_ptr = value;
	...
	ERS_ONE(("Named parameter #<%s> not defined"), paramNameBuf);
    }
    return INTERP_OK;
}
...
int Interp::fetch_ini_param( const char *nameBuf, int *status, double *value)
{
    ...
    *status = 0;
	...
	if (!inifile.Open(iniFileName)) {
	    *status = 0;
	    ERS_ONE(("cant open ini file '%s'"), iniFileName);
	}

	char capName[LINELEN];

	strncpy_s(capName, nameBuf, n);
	capName[n] = '\0';
	...
	if ((retval = inifile.Find( value, &capName[closeBracket+1], &capName[5])) == 0) {
	    *status = 1;
	    ...
	    inifile.Close();
	    *status = 0;
	    ERS_THR(("Named ini parameter #<%s> not found in inifile '%s': error=0x%x"),
    ...
    )
{
  ...
  if (pi == frame->named_params.end()) {
      ERS_ONE(("Internal error: Could not assign #<%s>"), nameBuf);
  } else {
      parameter_pointer pv = &pi->second;

      CHKS_TWO(((pv->attr & PA_GLOBAL)  && level),
	   "BUG: variable '%s' marked global, but assigned at level %d", nameBuf, level);

      if ((pv->attr & PA_READONLY) && !override_readonly) {
	  ERS_ONE(("Cannot assign to read-only parameter #<%s>"), nameBuf);
      } else {
	  pv->value = value;
	  pv->attr &= ~PA_UNSET;
    ...
    int attr)       //!< see PA_* defs in interp_internal.hh
{
    ...
    CHKS_ONE( add_named_param((char *) nameBuf, PA_READONLY|attr),
	  "adding r/o '%s'", nameBuf);
    CHKS_TWO(store_named_param(&_setup, (char *) nameBuf, value, OVERRIDE_READONLY),
			   ...
			   double *value)
{
	...
	*value = _task;
    ...
    default:
	ERS_TWO(("BUG: lookup_named_param(%s): unhandled index=%fn"),
	      nameBuf,index);
    }
    return INTERP_OK;
}
