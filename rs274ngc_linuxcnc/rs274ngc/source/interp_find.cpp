...
int Interp::unwrap_rotary(double *r, double sign_of, double commanded, double current, char axis) {
    double result;
    int neg = _copysign(1.0, sign_of) < 0.0;
    CHKS_TWO((sign_of <= -360.0 || sign_of >= 360.0), (("Invalid absolute position %5.2f for wrapped rotary axis %c")), sign_of, axis);
                      ...
                      double *u_p, double *v_p, double *w_p)
{
    ...
    if (block->g_modes[GM_MODAL_0] == G_53) {      /* distance mode is absolute in this case */
...
#endif
        CHKS_STR((block->radius_flag || block->theta_flag), ("Cannot use polar coordinates with G53"));
        ...
        if(block->radius_flag && block->theta_flag) {
            CHKS_STR((block->x_flag || block->y_flag), ("Cannot specify X or Y words with polar coordinate"));
            *px = block->radius * cos(D2R(block->theta));
            *py = block->radius * sin(D2R(block->theta));
        } else if(block->radius_flag) {
            double theta;
            CHKS_STR((block->x_flag || block->y_flag), ("Cannot specify X or Y words with polar coordinate"));
            CHKS_STR((*py == 0 && *px == 0), ("Must specify angle in polar coordinate if at the origin"));
            theta = atan2(*py, *px);
        ...
        *py = (comp && middle && s->plane == CANON_PLANE_XY) ? s->program_y: s->current_y;
        ...
        if(block->radius_flag) {
            double radius, theta;
            CHKS_STR((block->x_flag || block->y_flag), ("Cannot specify X or Y words with polar coordinate"));
            CHKS_STR((*py == 0 && *px == 0), ("Incremental motion with polar coordinates is indeterminate when at the origin"));
            theta = atan2(*py, *px);
            radius = _hypot(*py, *px) + block->radius;
            *px = radius * cos(theta);
            *py = radius * sin(theta);
        }
        ...
        if(block->theta_flag) {
            double radius, theta;
            CHKS_STR((block->x_flag || block->y_flag), ("Cannot specify X or Y words with polar coordinate"));
            CHKS_STR((*py == 0 && *px == 0), ("G91 motion with polar coordinates is indeterminate when at the origin"));
...
int Interp::find_tool_pocket(setup_pointer settings, int toolno, int *pocket)
{
    ...
    *pocket = -1;
    ...
    CHKS_ONE((*pocket == -1), (("Requested tool %d not found in the tool table")), toolno);
    return INTERP_OK;
}
