...
const char *Interp::interp_status(int status) {
    ...
    static const char *msgs[] = { "INTERP_OK", "INTERP_EXIT",
	    ...
	    "INTERP_ERROR" };
    sprintf_s(statustext, "%s%s%d", ((status >= INTERP_OK) && (status
		   ...
		   const char *fmt, ...)
{
	...
	fprintf_s(log_file, "%04d%02d%02d-%02d:%02d:%02d ",//.%03ld
...
void Interp::DBG(int level,const char *fmt, va_list & ap)
{
...
void Interp::MSG_DBG(const char *fmt,...)
{
	va_list ap;
	va_start(ap, fmt);
    DBG(0, fmt, ap);
...
void Interp::do_Log(unsigned int flags, const char *file, int line,
...
void Interp::_logDebug ( unsigned int mask, int dlflags,int level, const char *fmt, va_list & ap)
{
...
void Interp::logDebug (const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	_logDebug(EMC_DEBUG_INTERP,0,1,fmt, ap);
	va_end(ap);
}

void Interp::logRemap (const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	_logDebug(EMC_DEBUG_REMAP,0,1,fmt, ap);
	va_end(ap);
}

void Interp::logNP (const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	_logDebug(EMC_DEBUG_NAMEDPARAM,0,1,fmt, ap);
	va_end(ap);
}

void Interp::logPy (const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	_logDebug(EMC_DEBUG_PYTHON,0,1,fmt, ap);
	va_end(ap);
}

void Interp::logConfig (const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	_logDebug(EMC_DEBUG_CONFIG,0,1,fmt, ap);
	va_end(ap);
}

void Interp::logOword (const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	_logDebug(EMC_DEBUG_OWORD,0,1,fmt, ap);
	va_end(ap);
}
...
int Interp::_execute(const char *command)
{
  ...
  if (_setup.line_length != 0) {        /* line not blank */
	  ...
	  if (status < 0) {
		  ...
		  ERS_TWO("BUG: execute_block: got %d - not in remappings() !! (next_remap=%d)",- status,next_remap);
	      }
...
int Interp::remap_finished(int phase)
{
	...
	if (cblock->remappings.find(-phase) == cblock->remappings.end()) {
	    ERS_ONE("remap_finished: got %d - not in cblock.remappings!",phase);
	}
	...
	if (it != cblock->remappings.end()) {
		...
		ERS_STR("BUG - remappings not empty");
	    }
		...
		if (status < 0)
		    ERS_TWO("BUG - check find_remappings()!! status=%d nesting=%d",status,_setup.remap_level);
	    } else {
	...
	// "should not happen"
	ERS_TWO("BUG: remap_finished(): phase=%d nesting=%d",
...
int Interp::init()
{
  ...
  iniFileName = "rs274ngc.ini";//getenv("INI_FILE_NAME");
  ...
  if(iniFileName != NULL) {
	  ...
	  if ( getapppath ( szfile, PATH_MAX ) )
	  {
		  sprintf_s ( buffer, "%s\\%s", szfile, iniFileName );
	  }
      if (inifile.Open(buffer) == false) {
...
int Interp::load_tool_table()
{
  ...
  CHKS_STR((_setup.pockets_max > CANON_POCKETS_MAX), NCE_POCKET_MAX_TOO_LARGE);
...
int Interp::open(const char *filename) //!< string: the name of the input NC-program file
{
  ...
  CHKS_STR((_setup.file_pointer != NULL), NCE_A_FILE_IS_ALREADY_OPEN);
  CHKS_STR((strlen(filename) > (LINELEN - 1)), NCE_FILE_NAME_TOO_LONG);
  _setup.file_pointer = fopen(filename, "r");
  CHKS_ONE((_setup.file_pointer == NULL), NCE_UNABLE_TO_OPEN_FILE, filename);
  line = _setup.linetext;
  for (index = -1; index == -1;) {      /* skip blank lines */
    CHKS_STR((fgets(line, LINELEN, _setup.file_pointer) ==
...
int Interp::read_inputs(setup_pointer settings)
{
	...
	CHKS_STR((GET_EXTERNAL_QUEUE_EMPTY() == 0),
	...
	CHKS_STR((GET_EXTERNAL_QUEUE_EMPTY() == 0),
	     ("Queue is not empty after tool change"));
...
int Interp::_read(const char *command)  //!< may be NULL or a string to read
{
  ...
  if ((read_status == INTERP_EXECUTE_FINISH)
	...
	// mah: FIXME test this - no idea what this is about; o_number is history
      ...
      CHKS_ONE((_setup.skipping_o != NULL),
...
int Interp::restore_parameters(const char *filename)   //!< name of parameter file to read  
{
  ...
  CHKS_ONE((infile == NULL), ("Unable to open parameter file: '%s'"), filename);
  ...
  while (feof(infile) == 0) {
    ...
    if (sscanf(line, "%d %lf", &variable, &value) == 2) {
      ...
      for (; k < RS274NGC_MAX_PARAMETERS; k++) {
        if (k > variable) {
          fclose(infile);
          ERS_STR(NCE_PARAMETER_FILE_OUT_OF_ORDER);
        } else if (k == variable) {
          pars[k] = value;
          if (k == required)
            required = _required_parameters[index++];
                             ...
                             const double parameters[]) //!< parameters to save   
{
  ...
  CHKS_STR((outfile == NULL), NCE_CANNOT_OPEN_VARIABLE_FILE);
...
int Interp::synch()
{
  ...
  if ( getapppath ( szfile, PATH_MAX ) )
  {
	  sprintf_s ( buffer, "%s\\%s", szfile, ((file_name[0] == 0) ?
      RS274NGC_PARAMETER_FILE_NAME_DEFAULT :
	  file_name) );
	  save_parameters(buffer, _setup.parameters);
  }
...
void Interp::ERS_FUNC (const char *fmt, ...)
{
	///setError
	...
	/////
		...
		strncpy_s(_setup.stack[_setup.stack_index], __FUNCTION__, STACK_ENTRY_LEN);
...
void Interp::_setError (const char *fmt, va_list & ap)
{
	vsnprintf(savedError, LINELEN, fmt, ap);
}

void Interp::Error(const char *fmt, ...)
{
	va_list ap; 
	va_start(ap, fmt); 
	_logDebug(EMC_DEBUG_UNCONDITIONAL,0,-1,fmt, ap);
	va_end(ap);
}
void Interp::Log(const char *fmt, ...)
{
	va_list ap; 
	va_start(ap, fmt); 
...
int Interp::enter_remap(void)
{
	...
	_setup.remap_level = 0;
	ERS_STR("maximum nesting of remapped blocks execeeded");
    }
...
int Interp::leave_remap(void)
{
	...
	ERS_ONE("BUG: remap_level < 0 : %d",_setup.remap_level);
    }
    return INTERP_OK;
}
...
int Interp::on_abort(int reason, const char *message)
{
    ...
    _snprintf_s(cmd,sizeof(cmd), "%s [%d]",_setup.on_abort_command, reason);
