				  ...
				  int number)
{
    ...
    CHKS_STR((remap == NULL), "BUG: convert_remapped_code: no remapping");
    ...
    _snprintf_s(cmd, sizeof(cmd),"O <%s> call ", REMAP_FUNC(remap));
			   ...
			   char *posarglist)
{
    ...
    if (!rptr) {
	ERS_STR("BUG: add_parameters: remap_frame: executing_remap == NULL ");
    }
    code = rptr->name;
    ...
    CHKS_STR((argspec == NULL),"BUG: add_parameters: argspec = NULL");
...
#define STORE(name,value)						\
	...
	/*try {*/								\
	    ...
	    /*PyErr_Print();*/						\
	    /*PyErr_Clear();*/						\
	    ERS_ONE("add_parameters: cant add '%s' to args",name);		\
	/*}*/								\
    }									\
    if (posarglist) {							\
	char actual[LINELEN];						\
	_snprintf_s(actual, sizeof(actual),"[%.4lf]", value);		\
...
#define PARAM(spec,name,flag,val) 	                    	\
    if ((flag)) { /* present */	                    	        \
...
#define CHECK(bad, fmt, ...)			\
	...
	if (sscanf_s(code + 1, "%d.%d", &g1, &g2) == 2) {
	    gcode = g1 * 10 + g2;
	}
	if ( gcode == -1) {
	    if (sscanf_s(code + 1, "%d", &gcode) != 1) {
