                             ...
                             double spiral_rel_tolerance)
{
  ...
  {
	  ERS_FUNC ("Radius to end of arc differs from radius to start: "
...
//        abs_err, rel_err*100);
  ...
  CHKS_STR(((arc_radius <= tool_radius) && (((side == LEFT) && (move == G_3)) ||
                                       ((side == RIGHT) && (move == G_2)))),
  ...
  if (move == G_2)
    ...
    *turn = 1 * p_number;
    ...
    ERS_STR(NCE_BUG_CODE_NOT_G2_OR_G3);
  return INTERP_OK;
}
                           ...
                           double tolerance)    //!< tolerance of differing radii
{
  ...
  CHKS_STR(((abs_radius <= tool_radius) && (((side == LEFT) && (move == G_3)) ||
                                       ((side == RIGHT) && (move == G_2)))),
      NCE_TOOL_RADIUS_NOT_LESS_THAN_ARC_RADIUS_WITH_COMP);
                        ...
                        double spiral_rel_tolerance)
{
  ...
  radius2 = _hypot((*center_x - end_x), (*center_y - end_y));
  if ( ((radius < radius_tolerance) || (radius2 < radius_tolerance)) )
  {
	  ERS_FUNC ( ("Zero-radius arc: "
		  "start=(%c%.4f,%c%.4f) center=(%c%.4f,%c%.4f) end=(%c%.4f,%c%.4f) r1=%.4f r2=%.4f"),
		  a, current_x, b, current_y,
