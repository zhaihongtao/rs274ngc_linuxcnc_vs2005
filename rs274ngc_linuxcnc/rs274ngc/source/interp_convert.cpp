...
int Interp::comp_get_current(setup_pointer settings, double *x, double *y, double *z) {
    ...
    case CANON_PLANE_XZ:
        ...
        *z = settings->current_y;
        break;
    default:
        ERS_STR("BUG: Invalid plane in comp_get_current");
    }
    return INTERP_OK;
}
...
int Interp::comp_set_current(setup_pointer settings, double x, double y, double z) {
    ...
    case CANON_PLANE_XZ:
        ...
        break;
    default:
        ERS_STR("BUG: Invalid plane in comp_set_current");
    }
    return INTERP_OK;
}
...
int Interp::comp_get_programmed(setup_pointer settings, double *x, double *y, double *z) {
    ...
    case CANON_PLANE_XZ:
        ...
        *z = settings->program_y;
        break;
    default:
        ERS_STR("BUG: Invalid plane in comp_get_programmed");
    }
    return INTERP_OK;
}
...
int Interp::comp_set_programmed(setup_pointer settings, double x, double y, double z) {
    ...
    case CANON_PLANE_XZ:
        ...
        break;
    default:
        ERS_STR("BUG: Invalid plane in comp_set_programmed");
    }
    return INTERP_OK;
}
...
//        printf("*-----------------------------------------*\n");
        ...
        CHKS_STR((settings->motion_mode != G_5_2), (
             ("Cannot use G5.3 without G5.2 first")));
        CHKS_ONE((nurbs_control_points.size()<nurbs_order), ("You must specify a number of control points at least equal to the order L = %d"), nurbs_order);
       ...
       setup_pointer settings)  //!< pointer to machine settings
{
    ...
    CHKS_STR((settings->cutter_comp_side), ("Cannot convert spline with cutter radius compensation")); // XXX

    if (settings->feed_mode == UNITS_PER_MINUTE) {
      CHKS_STR((settings->feed_rate == 0.0),
        NCE_CANNOT_MAKE_ARC_WITH_ZERO_FEED_RATE);
    } else if (settings->feed_mode == INVERSE_TIME) {
      CHKS_STR((!block->f_flag),
        NCE_F_WORD_MISSING_WITH_INVERSE_TIME_ARC_MOVE);
    }

    CHKS_STR((settings->plane != CANON_PLANE_XY), ("Splines must be in the XY plane")); // XXX
       //Error (for now): Splines must be in XY plane

    CHKS_STR((block->z_flag || block->a_flag || block->b_flag
          || block->c_flag),
          ("Splines may not have motion in Z, A, B, or C"));

    if(mode == G_5_1) {
      CHKS_STR(!block->i_flag || !block->j_flag,
      ...
      CHKS_STR(!block->p_flag || !block->q_flag,
                       ...
                       setup_pointer settings)  //!< pointer to machine settings             
{
  ...
  CHKS_STR((settings->arc_not_allowed), (("The move just after exiting cutter compensation mode must be straight, not an arc")));

  ijk_flag = block->i_flag || block->j_flag || block->k_flag;
  first = settings->cutter_comp_firstmove;

  CHKS_STR((settings->plane == CANON_PLANE_UV
            || settings->plane == CANON_PLANE_VW
            || settings->plane == CANON_PLANE_UW),
    ("Cannot do an arc in planes G17.1, G18.1, or G19.1"));
  CHKS_STR(((!block->r_flag) && (!ijk_flag)),
      NCE_R_I_J_K_WORDS_ALL_MISSING_FOR_ARC);
  CHKS_STR(((block->r_flag) && (ijk_flag)),
      NCE_MIXED_RADIUS_IJK_FORMAT_FOR_ARC);
  if (settings->feed_mode == UNITS_PER_MINUTE) {
    ...
    CHKS_STR((settings->feed_rate == 0.0),
        NCE_CANNOT_MAKE_ARC_WITH_ZERO_FEED_RATE);
    CHKS_STR((settings->speed[settings->active_spindle] == 0.0),
  ...
  if (ijk_flag) {
    if (settings->plane == CANON_PLANE_XY) {
      CHKS_STR((block->k_flag), NCE_K_WORD_GIVEN_FOR_ARC_IN_XY_PLANE);
      if (!block->i_flag) { /* i or j flag on to get here */
	if (settings->ijk_distance_mode == MODE_ABSOLUTE) {
	  ERS_ONE(("%c word missing in absolute center arc"), 'I');
	} else {
	  block->i_number = 0.0;
	}
      } else if (!block->j_flag) {
	if (settings->ijk_distance_mode == MODE_ABSOLUTE) {
	  ERS_ONE(("%c word missing in absolute center arc"), 'J');
	} else {
	  block->j_number = 0.0;
	}
      }
    } else if (settings->plane == CANON_PLANE_YZ) {
      CHKS_STR((block->i_flag), NCE_I_WORD_GIVEN_FOR_ARC_IN_YZ_PLANE);
	...
	if (settings->ijk_distance_mode == MODE_ABSOLUTE) {
	  ERS_ONE(("%c word missing in absolute center arc"), 'K');
	} else {
	  block->k_number = 0.0;
	}
      }
    } else if (settings->plane == CANON_PLANE_XZ) {
      CHKS_STR((block->j_flag), NCE_J_WORD_GIVEN_FOR_ARC_IN_XZ_PLANE);
	...
	if (settings->ijk_distance_mode == MODE_ABSOLUTE) {
      ...
      ERS_STR(NCE_BUG_PLANE_NOT_XY_YZ_OR_XZ);
    }
  } else {
    // in R format, we need some XYZ words specified because a full circle is not allowed.
    if (settings->plane == CANON_PLANE_XY) { 
        CHKS_STR(((!block->x_flag) && (!block->y_flag) && (!block->radius_flag) && (!block->theta_flag)),
            NCE_X_AND_Y_WORDS_MISSING_FOR_ARC_IN_XY_PLANE);
    } else if (settings->plane == CANON_PLANE_YZ) {
        CHKS_STR(((!block->y_flag) && (!block->z_flag)),
            NCE_Y_AND_Z_WORDS_MISSING_FOR_ARC_IN_YZ_PLANE);
    } else if (settings->plane == CANON_PLANE_XZ) {
        CHKS_STR(((!block->x_flag) && (!block->z_flag)),
            NCE_X_AND_Z_WORDS_MISSING_FOR_ARC_IN_XZ_PLANE);
    }
  }
                              ...
                              double u_end, double v_end, double w_end) //!< uvw at end of arc
{
    ...
    CHKS_STR((_hypot((end_x - cx), (end_y - cy)) <= tool_radius),
    ...
    double b_len = _hypot(cy - end_y, cx - end_x) / 2.0;
    double AB_ang = atan2(center_y - end_y, center_x - end_x);
    double A_ang = atan2(cy - end_y, cx - end_x) - AB_ang;

    CHKS_STR((fabs(cos(A_ang)) < TOLERANCE_EQUAL), NCE_TOOL_RADIUS_NOT_LESS_THAN_ARC_RADIUS_WITH_COMP);
    ...
    CHKS_STR((fabs(_hypot(center_x-end_x,center_y-end_y) - 
              _hypot(center_x-cx,center_y-cy)) > spiral_abs_tolerance),
                              ...
                              double u, double v, double w) //!< uvw at end of arc
{
    ...
    side = settings->cutter_comp_side;
    tool_radius = settings->cutter_comp_radius;   /* always is positive */
    arc_radius = _hypot((centerx - end_x), (centery - end_y));
    ...
    if (((side == LEFT) && (move == G_3)) || ((side == RIGHT) && (move == G_2))) {
        ...
        CHKS_STR((arc_radius <= tool_radius),
            NCE_TOOL_RADIUS_NOT_LESS_THAN_ARC_RADIUS_WITH_COMP);
    } else {
    ...
    if (beta < -small || 
        ...
        if (qc().front().type != QARC_FEED) {
            ...
            if TOOL_INSIDE_ARC(side, turn) {
                ...
                CHKS_STR((l > 1.0 || l < -1.0), ("Arc move in concave corner cannot be reached by the tool without gouging"));
                if(turn > 0) {
            ...
            double oldrad = _hypot(prev.center2 - prev.end2, prev.center1 - prev.end1);
            double newrad;
            if TOOL_INSIDE_ARC(side, turn) {
            ...
            arc_cc = _hypot(prev.center2 - centery, prev.center1 - centerx);

            CHKS_STR((oldrad == 0 || arc_cc == 0), ("Arc to arc motion is invalid because the arcs have the same center"));
            a = (SQ(oldrad) + SQ(arc_cc) - SQ(newrad)) / (2 * oldrad * arc_cc);
            
            CHKS_STR((a > 1.0 || a < -1.0), (("Arc to arc motion makes a corner the compensated tool can't fit in without gouging")));
            pullback = acos(a);
                                ...
                                setup_pointer settings) //!< pointer to machine settings                   
{
  ...
  CHKS_STR((settings->cutter_comp_side),      /* not "== true" */
      NCE_CANNOT_CHANGE_AXIS_OFFSETS_WITH_CUTTER_RADIUS_COMP);
  CHKS_TWO((block->a_flag && settings->a_axis_wrapped &&
	(block->a_number <= -360.0 || block->a_number >= 360.0)),
       (("Invalid absolute position %5.2f for wrapped rotary axis %c")),
       block->a_number, 'A');
  CHKS_TWO((block->b_flag && settings->b_axis_wrapped &&
	(block->b_number <= -360.0 || block->b_number >= 360.0)),
       (("Invalid absolute position %5.2f for wrapped rotary axis %c")),
       block->b_number, 'B');
  CHKS_TWO((block->c_flag && settings->c_axis_wrapped &&
  ...
  if ((g_code == G_52) || (g_code == G_92)) {
    ...
    ERS_STR(NCE_BUG_CODE_NOT_IN_G52_G92_SERIES);
...
int Interp::convert_param_comment(char *comment, char *expanded, int len)
{
    ...
    while(*comment)
    {
        if(*comment == '#')
        {
            ...
            // skip over the '#'
            comment++;
            CHKS_STR((0 == *comment), NCE_NAMED_PARAMETER_NOT_TERMINATED);
			...
			if (FEATURE(NO_DOWNCASE_OWORD))
                ...
                if('>' != *comment)
                {
                    ERS_STR(NCE_NAMED_PARAMETER_NOT_TERMINATED);
                }
                else
                {
                    comment++;
            ...
            else
            {
                ...
                *expanded++ = '#';
                ...
                CHKS_STR((*comment == 0), NCE_NAMED_PARAMETER_NOT_TERMINATED);
                continue;
            }
		...
		double pvalue = equal(value, 0.0) ? 0.0 : value;
                int n = _snprintf_s(valbuf, VAL_LEN, "%lf", pvalue);
...
int Interp::convert_control_mode(int g_code,     //!< g_code being executed (G_61, G61_1, || G_64)
				...
				double naivecam_tolerance,    //tolerance for the naivecam
                                setup_pointer settings) //!< pointer to machine settings                 
{
  CHKS_STR((settings->cutter_comp_side),
  ...
  if (g_code == G_61) {
    ...
    ERS_STR(NCE_BUG_CODE_NOT_G61_G61_1_OR_G64);
  return INTERP_OK;
}
                                     ...
                                     setup_pointer settings)    //!< pointer to machine settings                  
{
  ...
  case G_59_3:
    ...
    break;
  default:
    ERS_STR(NCE_BUG_CODE_NOT_IN_RANGE_G54_TO_G593);
  }
                                       ...
                                       setup_pointer settings)  //!< pointer to machine settings             
{
  ...
  if (g_code == G_40) {
    ...
    ERS_STR("BUG: Code not G40, G41, G41.1, G42, G42.1");
                                          ...
                                          setup_pointer settings)       //!< pointer to machine settings              
{
  ...
  CHKS_STR((settings->plane != CANON_PLANE_XY && settings->plane != CANON_PLANE_XZ),
      NCE_RADIUS_COMP_ONLY_IN_XY_OR_XZ);
  CHKS_STR((settings->cutter_comp_side),
      NCE_CANNOT_TURN_CUTTER_RADIUS_COMP_ON_WHEN_ON);
  if(block->g_modes[GM_CUTTER_COMP] == G_41_1 || block->g_modes[GM_CUTTER_COMP] == G_42_1) {
      CHKS_ONE((!block->d_flag),
              ("G%d.1 with no D word"), block->g_modes[GM_CUTTER_COMP]/10 );
      radius = block->d_number_float / 2;
      if(block->l_number != -1) {
          CHKS_ONE((settings->plane != CANON_PLANE_XZ), ("G%d.1 with L word, but plane is not G18"), block->g_modes[GM_CUTTER_COMP]/10);
      ...
      if(!block->d_flag) {
          ...
          int tool;
          CHKS_ONE(!is_near_int(&tool, block->d_number_float),
                  ("G%d requires D word to be a whole number"),
                   block->g_modes[GM_CUTTER_COMP]/10);
          CHKS_STR((tool < 0), NCE_NEGATIVE_D_WORD_TOOL_RADIUS_INDEX_USED);
      ...
      CHKS_ONE((settings->plane != CANON_PLANE_XZ && orientation != 0 && orientation != 9), ("G%d with lathe tool, but plane is not G18"), block->g_modes[7]/10);
  }
                                 ...
                                 setup_pointer settings)        //!< pointer to machine settings                 
{
  if (g_code == G_90) {
    ...
    ERS_STR(NCE_BUG_CODE_NOT_G90_OR_G91);
  return INTERP_OK;
}
                             ...
                             setup_pointer settings)    //!< pointer to machine settings                 
{
  if (g_code == G_93) {
    ...
    ERS_STR("BUG: Code not G93, G94, or G95");
  return INTERP_OK;
}
...
int Interp::convert_savehome(int code, block_pointer block, setup_pointer s) {
    ...
    if(s->cutter_comp_side) {
        ERS_STR(("Cannot set reference point with cutter compensation in effect"));
    }
    ...
    if(code == G_28_1) {
        ...
        ERS_STR("BUG: Code not G28.1 or G38.1");
    }
    return INTERP_OK;
}
                        ...
                        setup_pointer settings) //!< pointer to machine settings             
{
  ...
  if (move == G_28) {
    ...
    ERS_STR(NCE_BUG_CODE_NOT_G28_OR_G30);
                                ...
                                setup_pointer settings) //!< pointer to machine settings                 
{
  if (g_code == G_20) {
    ...
    ERS_STR(NCE_BUG_CODE_NOT_G20_OR_G21);
  return INTERP_OK;
}
...
int Interp::gen_settings(double *current, double *saved, std::string &cmd)
{
	...
	if (saved[i] != current[i]) {
	    ...
	    case 1:
		_snprintf_s(buf,sizeof(buf)," F%.1f", saved[i]);
                cmd += buf;
		break;
	    case 2:
		_snprintf_s(buf,sizeof(buf)," S%.0f", saved[i]);
                cmd += buf;
		break;
	    }
	}
...
int Interp::gen_g_codes(int *current, int *saved, std::string &cmd)
{
	...
	if (val != current[i]) {
		...
		if (val != -1) { // FIXME not sure if this is correct!
		    ...
		    // not be reset
		    if (val % 10) {
			_snprintf_s(buf,sizeof(buf)," G%d.%d", val / 10, val % 10);
		    } else {
			_snprintf_s(buf,sizeof(buf)," G%d", val / 10);
		    }
                    cmd += buf;
		} else {
		    // so complain rather loudly
		    MSG_DBG("------ gen_g_codes BUG: index %d = -1!!\n",i);
		}
		break;
	    }
	}
...
int Interp::gen_m_codes(int *current, int *saved, std::string &cmd)
{
	...
	if (val != current[i]) {
	    ...
	    case 8: // feed hold
		if (val != -1) {  // unsure..
		    _snprintf_s(buf,sizeof(buf),"M%d\n", val);
		    cmd += buf;
		} else {
		    MSG_DBG("------ gen_m_codes: index %d = -1!!\n",i);
		}
		break;
	    }
	}
			    ...
			    int from_level)    //!< call level of context to restore from
{
    ...
    CHKS_TWO((from_level < settings->call_level),
	 (("BUG: cannot restore from a lower call level (%d) to a higher call level (%d)")),from_level,settings->call_level);
    CHKS_ONE((from_level < 0), (("BUG: restore from level %d !?")),from_level);
    CHKS_ONE((settings->call_level < 0), (("BUG: restore to level %d !?")),settings->call_level);
    ...
    if (settings->active_g_codes[5] != settings->sub_context[from_level].saved_g_codes[5]) {
        char buf[LINELEN];
	_snprintf_s(buf,sizeof(buf), "G%d",settings->sub_context[from_level].saved_g_codes[5]/10);
	CHKS_ONE(execute(buf) != INTERP_OK, ("M7x: restore_settings G20/G21 failed: '%s'"), cmd.c_str());
    }
	...
	// so split and execute each line
        ...
        strncpy_s(buf, size + 1, cmd.c_str(), sizeof(buf));
	char *last = buf;
	char *s;
	while ((s = strtok_s(last, "\n", &last)) != NULL) {
	    int status = execute(s);
	    if (status != INTERP_OK) {
		char currentError[LINELEN+1];
		strcpy_s(currentError,getSavedError());
		if (status) {  
			delete [] buf;
			ERS_TWO( ("M7x: restore_settings failed executing: '%s': %s"), s, currentError);               
		    }  
	    }
	}
		      ...
		      setup_pointer settings)    //!< pointer to machine settings
{
  ...
  if (IS_USER_MCODE(block,settings,5) &&
      ...
      CHKS_STR((settings->cutter_comp_side),
           (("Cannot set motion output with cutter radius compensation on")));  // XXX
      CHKS_STR((!block->p_flag), ("No valid P word with M62"));
      SET_MOTION_OUTPUT_BIT(round_to_int(block->p_number));
  } else if ((block->m_modes[5] == 63) && ONCE_M(5)) {
      CHKS_STR((settings->cutter_comp_side),
           (("Cannot set motion digital output with cutter radius compensation on")));  // XXX
      CHKS_STR((!block->p_flag), ("No valid P word with M63"));
      CLEAR_MOTION_OUTPUT_BIT(round_to_int(block->p_number));
  } else if ((block->m_modes[5] == 64) && ONCE_M(5)){
      CHKS_STR((settings->cutter_comp_side),
           (("Cannot set auxiliary digital output with cutter radius compensation on")));  // XXX
      CHKS_STR((!block->p_flag), ("No valid P word with M64"));
      SET_AUX_OUTPUT_BIT(round_to_int(block->p_number));
  } else if ((block->m_modes[5] == 65) && ONCE_M(5)) {
      CHKS_STR((settings->cutter_comp_side),
           (("Cannot set auxiliary digital output with cutter radius compensation on")));  // XXX
      CHKS_STR((!block->p_flag), ("No valid P word with M65"));
	...
	if (round_to_int(block->p_number) < 0) // safety check for negative words
	    ERS_STR(("invalid P-word with M66"));
	...
	CHKS_STR((ret == -1), NCE_DIGITAL_INPUT_INVALID_ON_M66);
	if (ret == 0) {
	...
	CHKS_STR((ret == -1), NCE_ANALOG_INPUT_INVALID_ON_M66);
	if (ret == 0) {
	    settings->input_flag = true;
    ...
    //Q-word = analog value
      CHKS_STR((settings->cutter_comp_side),
           (("Cannot set motion analog output with cutter radius compensation on")));  // XXX
      CHKS_STR((!block->e_flag) || (round_to_int(block->e_number) < 0), (("Invalid analog index with M67")));
      SET_MOTION_OUTPUT_VALUE(round_to_int(block->e_number), block->q_number);
  } else if ((block->m_modes[5] == 68)  && ONCE_M(5)) {
    //E-word = analog channel
    //Q-word = analog value
      CHKS_STR((settings->cutter_comp_side),
           (("Cannot set auxiliary analog output with cutter radius compensation on")));  // XXX
      CHKS_STR((!block->e_flag) || (round_to_int(block->e_number) < 0), (("Invalid analog index with M68")));
  ...
  if ((block->m_modes[6] != -1)  && ONCE_M(6)){
	  ...
	  if (IS_USER_MCODE(block,settings,6) && remapped_in_block) {
	      ...
	      CHKS_STR((toolno < 0), (("Need non-negative Q-word to specify tool number with M61")));
 ...
 if (IS_USER_MCODE(block,settings,7) && ONCE_M(7)) {
    return convert_remapped_code(block, settings, STEP_M_7, 'm',
     ...
     if (block->dollar_flag){
        CHKS_TWO((block->dollar_number >= settings->num_spindles || block->dollar_number < 0),
     ...
     if (block->dollar_flag){
        CHKS_STR((block->dollar_number >= settings->num_spindles || block->dollar_number < 0),
     ...
     if (block->dollar_flag){
      ...
      if (block->dollar_flag){
         CHKS_STR((block->dollar_number >= settings->num_spindles || block->dollar_number < 0),
             (("Spindle ($) number out of range in M19 Command")));
      }
      ...
      if (block->q_flag) {
	  CHKS_STR((block->q_number <= 0.0),(("Q word with M19 requires a value > 0")));
	 ...
	 if (_setup.call_level == 0) {
	     MSG_DBG("Warning - M73 at top level: nothing to return to; storing context anyway\n");
	 } else {
      ...
      CHKS_STR((!(_setup.sub_context[_setup.call_level].context_status & CONTEXT_VALID)),
           (("Cannot restore context from invalid stack frame - missing M70/M73?")));
...
if (IS_USER_MCODE(block,settings,9) && ONCE_M(9)) {
    ...
    CHKS_STR((settings->cutter_comp_side),
         (("Cannot enable overrides with cutter radius compensation on")));  // XXX
    ...
    CHKS_STR((settings->cutter_comp_side),
         (("Cannot disable overrides with cutter radius compensation on")));  // XXX
...
if ((block->m_modes[9] == 51)  && ONCE_M(9)){
	int e = -1;
	if (block->dollar_flag){
		CHKS_STR((block->dollar_number <= 0 || block->dollar_number >= settings-> num_spindles),
				(("Invalid spindle ($) number in M51 command")));
...
if (IS_USER_MCODE(block,settings,10) && ONCE_M(10)) {
    ...
    if (USER_DEFINED_FUNCTION[index - 100] == 0) {
      CHKS_ONE(1, NCE_UNKNOWN_M_CODE_USED,index);
    }
                           ...
                           setup_pointer settings)      //!< pointer to machine settings                 
{
  ...
  if (code == G_10) {
    ...
    ERS_STR(NCE_BUG_CODE_NOT_G4_G10_G28_G30_G52_G53_OR_G92_SERIES);
  return INTERP_OK;
}
                          ...
                          setup_pointer settings)       //!< pointer to machine settings              
{
  ...
  if (motion != G_0) {
      CHKS_ONE((ai), (("Indexing axis %c can only be moved with G0")), 'A');
      CHKS_ONE((bi), (("Indexing axis %c can only be moved with G0")), 'B');
      CHKS_ONE((ci), (("Indexing axis %c can only be moved with G0")), 'C');
  }

  int xyzuvw_flag = (block->x_flag || block->y_flag || block->z_flag ||
                     block->u_flag || block->v_flag || block->w_flag);

  CHKS_ONE((ai && (xyzuvw_flag || block->b_flag || block->c_flag)),
       (("Indexing axis %c can only be moved alone")), 'A');
  CHKS_ONE((bi && (xyzuvw_flag || block->a_flag || block->c_flag)),
       (("Indexing axis %c can only be moved alone")), 'B');
  CHKS_ONE((ci && (xyzuvw_flag || block->a_flag || block->b_flag)),
       (("Indexing axis %c can only be moved alone")), 'C');
  ...
  if (ai || bi || ci) {
    ...
    ERS_STR(NCE_BUG_UNKNOWN_MOTION_CODE);
  }
                          ...
                          setup_pointer settings)        //!< pointer to machine settings             
{
  ...
  CHKS_STR((settings->feed_rate == 0.0), NCE_CANNOT_PROBE_WITH_ZERO_FEED_RATE);
  CHKS_STR(settings->feed_mode == UNITS_PER_REVOLUTION,
	  ("Cannot probe with feed per rev mode"));
  CHP(find_ends(block, settings, &end_x, &end_y, &end_z,
                &AA_end, &BB_end, &CC_end,
                &u_end, &v_end, &w_end));
  CHKS_STR(((!(probe_type & 1)) && 
                                ...
                                setup_pointer settings) //!< pointer to machine settings                 
{
  ...
  if (g_code == G_98) {
    ...
    ERS_STR(NCE_BUG_CODE_NOT_G98_OR_G99);
  return INTERP_OK;
}
...
int Interp::convert_setup_tool(block_pointer block, setup_pointer settings) {
    ...
    CHKS_STR(!(block->x_flag || block->y_flag || block->z_flag ||
	   block->a_flag || block->b_flag || block->c_flag ||
	   block->u_flag || block->v_flag || block->w_flag ||
	   block->r_flag || block->q_flag || block->i_flag ||
           block->j_flag),
	 ("G10 L1 without offsets has no effect"));
    ...
    if(block->q_number != -1.0) {
        CHKS_STR((!is_near_int(&q, block->q_number)), ("Q number in G10 is not an integer"));
        CHKS_STR((q > 9), ("Invalid tool orientation"));
                         ...
                         setup_pointer settings)        //!< pointer to machine settings                 
{
  ...
  CHKS_STR((block->i_flag || block->j_flag), ("I J words not allowed with G10 L2"));
  ...
  CHKS_TWO((block->l_number == 20 && block->a_flag && settings->a_axis_wrapped && 
        (block->a_number <= -360.0 || block->a_number >= 360.0)), 
       (("Invalid absolute position %5.2f for wrapped rotary axis %c")), block->a_number, 'A');
  CHKS_TWO((block->l_number == 20 && block->b_flag && settings->b_axis_wrapped && 
        (block->b_number <= -360.0 || block->b_number >= 360.0)), 
       (("Invalid absolute position %5.2f for wrapped rotary axis %c")), block->b_number, 'B');
  CHKS_TWO((block->l_number == 20 && block->c_flag && settings->c_axis_wrapped && 
        (block->c_number <= -360.0 || block->c_number >= 360.0)), 
       (("Invalid absolute position %5.2f for wrapped rotary axis %c")), block->c_number, 'C');

  CHKS_STR((settings->cutter_comp_side && p_int == settings->origin_index),
  ...
  if (block->r_flag) {
    CHKS_STR((block->l_number == 20), ("R not allowed in G10 L20"));
    r = block->r_number;
...
int Interp::convert_set_plane(int g_code,        //!< must be G_17, G_18, or G_19 
                             setup_pointer settings)    //!< pointer to machine settings 
{
  CHKS_STR((settings->cutter_comp_side && g_code == G_17 && settings->plane != CANON_PLANE_XY),
        NCE_CANNOT_CHANGE_PLANES_WITH_CUTTER_RADIUS_COMP_ON);
  CHKS_STR((settings->cutter_comp_side && g_code == G_18 && settings->plane != CANON_PLANE_XZ),
        NCE_CANNOT_CHANGE_PLANES_WITH_CUTTER_RADIUS_COMP_ON);
  CHKS_STR((settings->cutter_comp_side && g_code == G_19 && settings->plane != CANON_PLANE_YZ),
        NCE_CANNOT_CHANGE_PLANES_WITH_CUTTER_RADIUS_COMP_ON);

  CHKS_STR((settings->cutter_comp_side && g_code == G_19), 
          NCE_RADIUS_COMP_ONLY_IN_XY_OR_XZ);
  ...
  if (g_code == G_17) {
    ...
    ERS_STR(NCE_BUG_CODE_NOT_G17_G18_OR_G19);
  return INTERP_OK;
}
                        ...
                        setup_pointer settings) //!< pointer to machine settings                 
{
  ...
  CHKS_STR((block->m_modes[4] == 99 && settings->call_level > 0),
...
/*10*/
    ...
    ERS_STR(NCE_BUG_CODE_NOT_M0_M1_M2_M30_M60_M99);
  return INTERP_OK;
}
                            ...
                            setup_pointer settings)     //!< pointer to machine settings             
{
  ...
  if (move == G_1) {
    if (settings->feed_mode == UNITS_PER_MINUTE) {
      CHKS_STR((settings->feed_rate == 0.0), NCE_CANNOT_DO_G1_WITH_ZERO_FEED_RATE);
    } else if (settings->feed_mode == UNITS_PER_REVOLUTION) {
      CHKS_STR((settings->feed_rate == 0.0), NCE_CANNOT_DO_G1_WITH_ZERO_FEED_RATE);
      CHKS_STR((settings->speed[settings->active_spindle] == 0.0),
    		  (("Cannot feed with zero spindle speed in feed per rev mode")));
  ...
  if ((settings->cutter_comp_side) &&    /* ! "== true" */
    ...
    CHKS_STR((block->g_modes[GM_MODAL_0] == G_53),
        NCE_CANNOT_USE_G53_WITH_CUTTER_RADIUS_COMP);
    ...
    if(settings->plane == CANON_PLANE_XZ) {
        ...
        if (settings->cutter_comp_firstmove)
            ...
            status = convert_straight_comp2(move, block, settings, end_x, end_y, end_z,
                                            AA_end, BB_end, CC_end, u_end, v_end, w_end);
    } else ERS_STR("BUG: Invalid plane for cutter compensation");
    CHP(status);
  } else if (move == G_0) {
    STRAIGHT_TRAVERSE(block->line_number, end_x, end_y, end_z,
                      AA_end, BB_end, CC_end,
	...
	if (block->dollar_flag){
		CHKS_STR((block->dollar_number < 0 || block->dollar_number >= settings->num_spindles),
				(("Invalid spindle ($) number in G33 move")));
		settings->active_spindle = (int)block->dollar_number;
	}
    CHKS_STR(((settings->spindle_turning[settings->active_spindle] != CANON_CLOCKWISE) &&
	...
	if (block->dollar_flag){
		CHKS_STR((block->dollar_number < 0 || block->dollar_number >= settings->num_spindles),
				(("Invalid spindle ($) number in G33.1 move")));
		settings->active_spindle = (int)block->dollar_number;
	}
    CHKS_STR(((settings->spindle_turning[settings->active_spindle] != CANON_CLOCKWISE) &&
           (settings->spindle_turning[settings->active_spindle] != CANON_COUNTERCLOCKWISE)),
	...
	if (block->dollar_flag){
		CHKS_STR((block->dollar_number < 0 || block->dollar_number >= settings->num_spindles),
				(("Invalid D-number in G76 cycle")));
		settings->active_spindle = (int)block->dollar_number;
	}
    CHKS_ONE(((settings->spindle_turning[settings->active_spindle] != CANON_CLOCKWISE) &&
           (settings->spindle_turning[settings->active_spindle] != CANON_COUNTERCLOCKWISE)),
          ("Chosen spindle (%i) not turning in G76"), settings->active_spindle);
    CHKS_STR((settings->AA_current != AA_end || 
         settings->BB_current != BB_end || 
         settings->CC_current != CC_end ||
         settings->u_current != u_end ||
         settings->v_current != v_end ||
    ...
    ERS_STR(NCE_BUG_CODE_NOT_G0_OR_G1);
...
int Interp::convert_straight_indexer(int axis, int jnum, block_pointer block, setup_pointer settings) {
    ...
    CHKS_STR((end_x != settings->current_x ||
         ...
         ("BUG: An axis incorrectly moved along with an indexer"));
    ...
    case 5:
        ...
        break;
    default:
        ERS_STR((("BUG: trying to index incorrect axis")));
    }
    return INTERP_OK;
}
...
int Interp::convert_threading_cycle(block_pointer block, 
    ...
    CHKS_STR((block->i_number == 0),
        (("In G76, I must not be 0")));
    CHKS_STR((block->j_number <= 0),
        (("In G76, J must be greater than 0")));
    CHKS_STR((block->k_number <= block->j_number),
        (("In G76, K must be greater than J")));
	...
	pitch * _hypot(taper_dist, full_threadheight)/taper_dist: pitch;
                                   ...
                                   double u_end, double v_end, double w_end)
{
    ...
    CHKS_STR(((side != LEFT) && (side != RIGHT)), NCE_BUG_SIDE_NOT_RIGHT_OR_LEFT);
    CHKS_STR((distance <= radius), ("Length of cutter compensation entry move is not greater than the tool radius"));
                                   ...
                                   double u_end, double v_end, double w_end)
{
    ...
    if ((py == opy) && (px == opx)) {     /* no XY motion */
        ...
        if (side == LEFT) {
            ...
            ERS_STR(NCE_BUG_SIDE_NOT_RIGHT_OR_LEFT);
        ...
        if (!concave && (beta > small)) {       /* ARC NEEDED */
            ...
            if(move == G_1) {
                ...
                dequeue_canons(settings);
                set_endpoint(mid_x, mid_y);
            } else ERS_STR(NCE_BUG_CODE_NOT_G0_OR_G1);
        } else if (concave) {
            if (qc().front().type != QARC_FEED) {
                // line->line
                double retreat;
                ...
                CHKS_STR((halfcorner == 0.0), (("Zero degree inside corner is invalid for cutter compensation")));
                ...
                if TOOL_INSIDE_ARC(side, prev.turn) {
                    d2 = d - radius;
                    double l = d2/oldrad;
                    CHKS_STR((l > 1.0 || l < -1.0), ("Arc to straight motion makes a corner the compensated tool can't fit in without gouging"));
                    if(prev.turn > 0)
...
int Interp::convert_tool_change(setup_pointer settings)  //!< pointer to machine settings
{
  ...
  if (settings->selected_pocket < 0) {
    ERS_STR(NCE_TXX_MISSING_FOR_M6);
  }
                                      ...
                                      setup_pointer settings)   //!< pointer to machine settings                 
{
  ...
  if (g_code == G_49) {
    ...
    CHKS_STR((!block->h_flag), (("G43.2: H-word missing")));
    CHP((find_tool_pocket(settings, block->h_number, &pocket_number)));
    tool_offset = settings->tool_offset;
    tool_offset.tran.x += USER_TO_PROGRAM_LEN(settings->tool_table[pocket_number].offset.tran.x);
    tool_offset.tran.y += USER_TO_PROGRAM_LEN(settings->tool_table[pocket_number].offset.tran.y);
    ...
    ERS_STR("BUG: Code not G43, G43.1, G43.2, or G49");
  }
