				...
				setup_pointer settings)     /* pointer to machine settings */
{
	...
	if (settings->sequence_number -1 == op->sequence_number)
	    ...
	    ERS_THR(("File:%s line:%d sub: o|%s| found in illegal location"),
		settings->filename, settings->sequence_number, block->o_name);
	else
	    // already exists
	    ERS_FOR(("File:%s line:%d redefining sub: o|%s| already defined in file:%s"),
			       ...
			       offset_pointer *op)       // pointer to offset descriptor
{
	...
	*op =  &it->second;
	...
	ERS_STR(NCE_UNKNOWN_OWORD_NUMBER);
    }
}
...
int Interp::execute_return(setup_pointer settings, context_pointer current_frame,int call_type)
{
	...
	if (settings->call_level != 0) {
		...
		if (0 != strcmp(settings->filename, previous_frame->filename))  {
		    ...
		    if (settings->file_pointer == NULL)  {
			ERS_TWO(NCE_CANNOT_REOPEN_FILE, 
			    previous_frame->filename,
			    strerror(errno));
		    }
		    strcpy_s(settings->filename, previous_frame->filename);
		}
		...
		CHKS_STR((settings->defining_sub != 1), NCE_NOT_IN_SUBROUTINE_DEFN);
			     ...
			     setup_pointer settings)   // pointer to machine settings
{
	...
	if (0 != strcmp(settings->filename,
	    ...
	    // set the line number
	    settings->sequence_number = 0;
            strncpy_s(settings->filename, op->filename, sizeof(settings->filename));
            if (settings->filename[sizeof(settings->filename)-1] != '\0') {
                fclose(settings->file_pointer);
                logOword("filename too long: %s", op->filename);
                ERS_ONE(NCE_UNABLE_TO_OPEN_FILE, op->filename);
            }
		...
		ERS_ONE(NCE_UNABLE_TO_OPEN_FILE,settings->filename);
	    }
	}
	...
	if (settings->file_pointer)
	    fclose(settings->file_pointer);
	settings->file_pointer = newFP;
        strncpy_s(settings->filename, newFileName, sizeof(settings->filename));
        if (settings->filename[sizeof(settings->filename)-1] != '\0') {
...
int Interp::leave_context(setup_pointer settings, bool restore) 
{
   ...
   if (settings->call_level < 1) {
	ERS_STR(NCE_CALL_STACK_UNDERRUN);
    }
				      ...
				      setup_pointer settings)  // pointer to machine settings
{
	...
	if (settings->call_level != 0) {
	    ...
	    logOword("started a rs274-style sub-program defn: %s",
		     block->o_name);
	    CHKS_STR((settings->defining_sub == 1), NCE_NESTED_SUBROUTINE_DEFN);
	    CHP(control_save_offset( block, settings));
	...
	CHKS_STR((block->o_type == M_99 && settings->call_level == 0),
	...
	if (control_find_oword(block, settings, &op) == INTERP_OK) {
	    if (settings->sequence_number != (op->sequence_number + 1)) 
	        OERR_THR(("%d: duplicate O-word label: '%s' - defined in line %d"),
	...
	if (control_find_oword(block, settings, &op) == INTERP_OK) {
	    if (settings->sequence_number != (op->sequence_number + 1)) 
	        OERR_THR(("%d: duplicate O-word label - already defined in line %d: '%s'"),
		    settings->sequence_number, op->sequence_number + 1,
		    settings->linetext);
	} else 
	    CHP(control_save_offset(block, settings));
    ...
    case O_elseif:
	if (control_find_oword(block, settings, &op) != INTERP_OK) 
	    OERR_TWO(("%d: undefined O-word label: '%s'"),
		settings->sequence_number, settings->linetext);
	if (op->type != O_if)
	    OERR_FOR(("%d: no matching 'if' label: '%s' (found '%s' in line %d)"),
		settings->sequence_number, settings->linetext, 
	...
	if (control_find_oword(block, settings, &op) != INTERP_OK) 
		...
		settings->sequence_number, settings->linetext);
	if (op->type != O_if)
	    OERR_FIV(("%d: no matching label: '%s' (found '%s' in line %d): '%s'"),
	...
	if ((op->type != O_while) && (op->type != O_do))
	    OERR_FOR(("%d: no matching while/do label: '%s' (found '%s' in line %d)"),
		settings->sequence_number, settings->linetext, 
		o_ops[op->type] + 2, op->sequence_number + 1);
	...
	if (((block->o_type == O_endrepeat) && (op->type != O_repeat)) ||
	    ...
	    OERR_FOR(("%d: no matching label: '%s' (found '%s' in line %d)"),
		settings->sequence_number, settings->linetext, 
		o_ops[op->type] + 2, op->sequence_number + 1);
