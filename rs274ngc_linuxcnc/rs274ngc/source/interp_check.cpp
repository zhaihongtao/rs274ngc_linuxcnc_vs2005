                         ...
                         setup_pointer settings)        //!< pointer to machine settings
{
  ...
  if (mode0 == -1) {
    ...
    CHKS_STR((block->p_number == -1.0), NCE_DWELL_TIME_MISSING_WITH_G4);
    CHKS_STR((mode1 == G_2 || mode1 == G_3), ("G4 not allowed with G2 or G3 because they both use P"));
  } else if (mode0 == G_10) {
    (block->p_number >= 0) ? p_int = (int) (block->p_number +0.5) :p_int = (int) (block->p_number -0.5);
    CHKS_STR((block->l_number != 2 && block->l_number != 1 && block->l_number != 20 && block->l_number != 10 && block->l_number != 11), ("Line with G10 does not have L1, L10, L11, L2, or L20"));
    CHKS_STR((((block->p_number + 0.0001) - p_int) > 0.0002),  ("P value not an integer with G10"));
    CHKS_ONE((((block->l_number == 2 || block->l_number == 20) && ((p_int < 0) || (p_int > 9)))), ("P value out of range (0-9) with G10 L%d"), block->l_number);
    CHKS_ONE((((block->l_number == 1 || block->l_number == 10 || block->l_number == 11) && p_int < 1)), ("P value out of range with G10 L%d"), block->l_number);
  } else if (mode0 == G_28) {
  } else if (mode0 == G_30) {
  } else if (mode0 == G_5_3) { 
      CHKS_STR(((mode1 != G_5_2) && (mode1 != -1)), ("Between G5.2 and G5.3 codes, only additional G5.2 codes are allowed."));
  } else if (mode1 == G_5_2){
  } else if (mode0 == G_28_1 || mode0 == G_30_1) {
  } else if (mode0 == G_52) {
  } else if (mode0 == G_53) {
    CHKS_STR(((block->motion_to_be != G_0) && (block->motion_to_be != G_1)),
        NCE_MUST_USE_G0_OR_G1_WITH_G53);
    CHKS_STR(((block->g_modes[GM_DISTANCE_MODE] == G_91) ||
    ...
    ERS_STR(NCE_BUG_BAD_G_CODE_MODAL_GROUP_0);
  return INTERP_OK;
}
...
int Interp::check_m_codes(block_pointer block)   //!< pointer to a block to be checked
{
  ...
  CHKS_STR((block->m_count > MAX_EMS), NCE_TOO_MANY_M_CODES_ON_LINE);
  return INTERP_OK;
}
...
int Interp::check_other_codes(block_pointer block)       //!< pointer to a block of RS274/NGC instructions
{
  ...
  if (block->a_flag) {
    CHKS_STR(is_a_cycle(motion), NCE_CANNOT_PUT_AN_A_IN_CANNED_CYCLE);
  }
  if (block->b_flag) {
    CHKS_STR(is_a_cycle(motion), NCE_CANNOT_PUT_A_B_IN_CANNED_CYCLE);
  }
  if (block->c_flag) {
    CHKS_STR(is_a_cycle(motion), NCE_CANNOT_PUT_A_C_IN_CANNED_CYCLE);
  }
  if (block->d_flag) {
    CHKS_STR(((block->g_modes[7] != G_41) && (block->g_modes[7] != G_42) &&
        (block->g_modes[7] != G_41_1) && (block->g_modes[7] != G_42_1) &&
        (block->g_modes[14] != G_96)),
        ("D word with no G41, G41.1, G42, G42.1, or G96 to use it"));
  }

  if (block->dollar_flag) {
      CHKS_STR(((motion != G_76) && (motion != G_33) && (motion != G_33_1) &&
      (motion != G_95) && (motion != G_96) && (motion != G_97) &&
      (block->m_modes[7] != 3) && (block->m_modes[7] != 4) &&
      (block->m_modes[7] != 5) && (block->m_modes[7] != 19) &&
      (! block->s_flag)),
      ("$ (spindle selection) word with no G41, G41.1, G42, G42.1, G76 G95 or G96 to use it"));
  }
  ...
  if (block->e_flag) {
    CHKS_STR(((motion != G_76) && (motion != G_33) && (motion != G_33_1) &&
      ...
      (block->m_modes[5] != 67) && (block->m_modes[5] != 68)),
       ("E word with no G76, M3, M4, M5, M19, M51, M66, M67 or M68 to use it"));
  }

  if (block->h_flag) {
    CHKS_STR((block->g_modes[GM_TOOL_LENGTH_OFFSET] != G_43 && motion != G_76 && block->g_modes[GM_TOOL_LENGTH_OFFSET] != G_43_2),
      ("H word with no G43 or G76 to use it"));
  }

  if (block->i_flag) {    /* could still be useless if yz_plane arc */
    CHKS_STR(((motion != G_2) && (motion != G_3) && (motion != G_5) && (motion != G_5_1) &&
  ...
  if (block->j_flag) {    /* could still be useless if xz_plane arc */
    CHKS_STR(((motion != G_2) && (motion != G_3) && (motion != G_5) && (motion != G_5_1) && 
        ...
        ("J word with no G2, G3, G5, G5.1, G10, G76 or G87 to use it"));
  }

  if (block->k_flag) {    /* could still be useless if xy_plane arc */
    CHKS_STR(((motion != G_2) && (motion != G_3) && (motion != G_33) &&
        (motion != G_33_1) && (motion != G_76) && (motion != G_87)),
        ("K word with no G2, G3, G33, G33.1, G76, or G87 to use it"));
  }

  if (block->l_number != -1) {
    CHKS_STR((((motion < G_81) || (motion > G_89)) && (motion != G_76) &&
	 ...
	 (block->o_type != M_98)  // m98 repeat
         ...
         ("L word with no G10, cutter compensation, canned cycle, "
	   "digital/analog input, M98 or NURBS code"));
  }

  if (block->p_flag) {
      CHKS_STR(((block->g_modes[GM_MODAL_0] != G_10) && (block->g_modes[GM_MODAL_0] != G_4) && (block->g_modes[GM_CONTROL_MODE] != G_64) &&
	  ...
	  (motion != G_74) && (motion != G_84) &&
          ...
          (block->o_type != M_98)),
          ("P word with no G2 G3 G4 G10 G64 G5 G5.2 G76 G82 G86 G88 G89"
            " or M50 M51 M52 M53 M62 M63 M64 M65 M66 M98 "
            "or user M code to use it"));
      int p_value = round_to_int(block->p_number);
      CHKS_STR(((motion == G_2 || motion == G_3 || (block->m_modes[7] == 19)) && 
	    fabs(p_value - block->p_number) > 0.001),
	   ("P value not an integer with M19 G2 or G3"));
      CHKS_STR((block->m_modes[7] == 19) && ((p_value > 2) || p_value < 0),
	   ("P value must be 0,1,or 2 with M19"));
      CHKS_STR(((motion == G_2 || motion == G_3) && round_to_int(block->p_number) < 1),
          ("P value should be 1 or greater with G2 or G3"));
  }

  if (block->q_number != -1.0) {
      CHKS_STR((motion != G_83) && (motion != G_73) && (motion != G_5) && (block->user_m != 1) && (motion != G_76) &&
	   ...
	   (block->m_modes[7] != 19), 
	   ("Q word with no G5, G10, G64, G73, G76, G83, M19, M66, M67, M68 or user M code that uses it"));
  }

  if (block->r_flag) {
    CHKS_STR(((motion != G_2) && (motion != G_3) && (motion != G_76) &&
	 ...
	 (motion != G_74) &&
        ...
        NCE_R_WORD_WITH_NO_G_CODE_THAT_USES_IT);
    CHKS_STR((block->m_modes[7] == 19) && ((block->r_number > 360.0) || (block->r_number < 0.0)),
	   ("R value must be within 0..360 with M19"));
  }

  if (!block->s_flag) {
    CHKS_STR((block->g_modes[GM_SPINDLE_MODE] == G_96), NCE_S_WORD_MISSING_WITH_G96);
  }

  if (motion == G_33 || motion == G_33_1) {
    CHKS_STR((!block->k_flag), NCE_K_WORD_MISSING_WITH_G33);
    CHKS_STR((block->f_flag), NCE_F_WORD_USED_WITH_G33);
  }

  if (motion == G_76) {
    // pitch
    CHKS_STR((block->p_number == -1), NCE_P_WORD_MISSING_WITH_G76);

    CHKS_STR((!block->i_flag || !block->j_flag || !block->k_flag),
